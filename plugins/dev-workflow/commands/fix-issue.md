...
For all modes, load appropriate skills (blazor for Blazor components, frontend-design for UI) and choose the right specialist agent type (`dotnet-specialist`, `dotnet-fixer`, `database-specialist`, etc.). Specialist agents come from the dev-agents plugin — if a named type is unavailable, or the project isn't .NET (e.g. a Python one-off), use `general-purpose`.
